...
# 
def rank_sim(num_docs, q_tokens, tf, idf):
	estm_len = [estimate_len( tf, doc) for doc in range(num_docs)]
	# print estm_len
	...
	for doc in range(num_docs):
		tokens_list = [t for t in q_tokens if tf[t][doc] >= 1]
		# print str(doc) + ' ' + str(tokens_list)
		doc_sim = 0
		j = doc
		for token in tokens_list:
			b = bm25(tf[token][j], estm_len[j], avg_len)
			sub_sentence = (num_docs - idf[token] + 0.5) / (idf[token] + 0.5)
...
if __name__ == '__main__':
	...
	text = ['O peã e o caval são pec de xadrez. O caval é o melhor do jog.', 'A jog envolv a torr, o peã e o rei.',
	 		'O peã lac o boi', 'Caval de rodei!', 'Polic o jog no xadrez.']
	...
	# print q_tokens
	# tk_docs = [tokenize(doc) for doc in text]
	# print 'test ' + str(tk_docs)
	rank_prob = rank_sim(len(text), query, tf, idf)

	print "Rank \t| Doc No."
	for rank in rank_prob:
		if rank[0] > 0:	
			print str(rank[0]) + '\t| ' + str(rank[1] + 1)
